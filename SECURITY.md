...
| **Arbitrary code execution** (the `run_command` shell tool) | **Off by default** (`jotflow.tools.shell`). When enabled it runs **only in a trusted Workspace**, and **each command requires a modal confirmation** (`jotflow.tools.shellAutoApprove` opts out, with a documented warning). It is not even offered to the model unless enabled. Output is capped; the process is tree-killed on timeout (60 s) or Stop. |

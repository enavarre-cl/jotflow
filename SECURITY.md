...
| **Path traversal** in `systemPromptFile` (reading an arbitrary file into the system prompt) | The path is **confined to the `.chat`'s directory**, both when resolving it for the model (`resolveSystemPrompt`) and when opening it in the editor (`openSysPrompt`). |
